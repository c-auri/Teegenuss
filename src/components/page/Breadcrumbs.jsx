    ...
    return <div className="flex gap-5">
        {
            navigation?.segments.map((segment, segmentIndex) => (
                <div key={`segment-${segmentIndex}`} className="group flex items-center select-none tracking-wider text-md text-slate-500 fill-slate-500">
                    <div className="relative cursor-pointer rounded-md hover:fill-slate-500 group-hover:text-slate-500 group-hover:bg-sky-100">
                        <div className="pl-2 pr-4 py-1 rounded-md flex items-center gap-1 hover:bg-sky-200">
...
function Dropdown(navigation, segmentIndex, currentEntry) {
    ...
    return collection.map((entry, entryIndex) => {
        ...
        return <a 
            ...
            key={`${entryIndex}`}
        >
            <li className={`${rounded} py-1 pl-3 pr-6 flex items-center gap-2 bg-sky-100 hover:text-slate-500 hover:bg-sky-200`}>
